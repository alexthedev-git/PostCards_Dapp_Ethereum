...
export default async function mintNFT(connection, wallet, files, metadata, sendTo) {
  ...
  if(sendTo.includes('.sol')){
    try{
      ...
      Vue.toasted.show('Can not fetch domain data', {
        icon: 'timer-sand',
        iconPack: 'mdi',
      });
  ...
  try{
    ...
    if (metadataFile?.transactionId && wallet.publicKey) {
      ...
      Vue.toasted.show('NFT created!', {
        ...
        action: [
          {
            text: 'View',
            href: `https://sonar.watch/collectibles/${(new PublicKey(sendTo)).toString()}`,
            target: '_blank',
          },
        ],
      });
    ...
    Vue.toasted.show('Error occured while minting...', {
      icon: 'timer-sand',
      iconPack: 'mdi',
    });
  }
